...
namespace AutoStorage
{
    public partial class Storage2 : MetroFramework.Forms.MetroForm
    {
        ...
        private void Storage2_Load(object sender, EventArgs e)
        {
            ...
            da.SelectCommand = new SqlCommand("SELECT * FROM Storage2", sqlConnection);
            da.Fill(ds);
            ...
            store2CongPickerBox.Text = metroGrid2.CurrentRow.Cells[2].Value.ToString();
            ...
            ////////////////////////////////////////////////////////////////////
            int a = Convert.ToInt32(store2CongPickerBox.Text);
        ...
        private void fromStorage2Btn_Click(object sender, EventArgs e) //кнопка вызывающая ф-цию удаления+ обновляет грид
        {
            goFromStorage();
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter();
        ...
        {
            SqlCommand goFromStrg1 = new SqlCommand("UPDATE StoragesCongestion SET Storage2 = Storage2 + @cubage where id=1", sqlConnection);
            ...
            SqlCommand goFromStrg2 = new SqlCommand("DELETE FROM Storage2 WHERE id=@id", sqlConnection);
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage2] WHERE (keepingSumBox LIKE '%" + priceSearchBox.Text + "%')", sqlConnection);
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage2] WHERE (companyFrom LIKE N'%" + typeSearchBox.Text.ToString() + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage2] WHERE (date1 LIKE '%" + dateSearchBox.Text + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
