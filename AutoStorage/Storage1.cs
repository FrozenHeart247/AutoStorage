...
namespace AutoStorage
{
    public partial class Storage1 : MetroFramework.Forms.MetroForm
    {
        ...
        private void Storage1_Load(object sender, EventArgs e)
        {
            ...
            da.SelectCommand = new SqlCommand("SELECT * FROM Storage1", sqlConnection); //Указываем номер склада
            da.Fill(ds);
            ...
            store1CongPickerBox.Text = metroGrid2.CurrentRow.Cells[1].Value.ToString(); //указываем номер склада
            ...
            int a = Convert.ToInt32(store1CongPickerBox.Text);
            int b=650; //Указываем размер склада
            int c;
            c = b - a;
            congestionPG.Value = c;
            if (c >= 650) //Указываем размер склада
            {
        ...
        private void fromStorage1Btn_Click(object sender, EventArgs e) //кнопка вызывающая ф-цию удаления+ обновляет грид
        {
            goFromStorage();
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter();
        ...
        public void goFromStorage()
        {
            if (idBox.Text == "")
            {
                MetroMessageBox.Show(this, "Ошибка", "Выберите накладную", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                SqlCommand goFromStrg1 = new SqlCommand("UPDATE StoragesCongestion SET Storage1 = Storage1 + @cubage where id=1", sqlConnection); //Указываем номер склада
                ...
                SqlCommand goFromStrg2 = new SqlCommand("DELETE FROM Storage1 WHERE id=@id", sqlConnection); //Указываем номер склада
        ...
        private void bunifuFlatButton6_Click(object sender, EventArgs e) //не работает
        {
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = new SqlCommand("SELECT * FROM Storage1", sqlConnection); 
            da.Fill(ds);
            metroGrid1.DataSource = ds.Tables[0];
            store1CongPickerBox.Text = metroGrid2.CurrentRow.Cells[1].Value.ToString();
            congestionPG.Refresh();
        }
        ...
        private void dateSearchBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage1] WHERE (date1 LIKE '%" + dateSearchBox.Text + "%')", sqlConnection);
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage1] WHERE (companyFrom LIKE N'%" + typeSearchBox.Text.ToString() + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage1] WHERE (keepingSumBox LIKE '%" + priceSearchBox.Text + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
