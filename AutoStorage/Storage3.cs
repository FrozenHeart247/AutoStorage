...
namespace AutoStorage
{
    public partial class Storage3 : MetroFramework.Forms.MetroForm
    {
        ...
        private void Storage3_Load(object sender, EventArgs e)
        {
            ...
            da.SelectCommand = new SqlCommand("SELECT * FROM Storage3", sqlConnection); //Указываем номер склада
            da.Fill(ds);
            ...
            store3CongPickerBox.Text = metroGrid2.CurrentRow.Cells[3].Value.ToString(); //указываем номер склада
            ...
            //////////////////////////////////////////
            int a = Convert.ToInt32(store3CongPickerBox.Text);
            int b = 400; //Указываем размер склада
            int c;
            c = b - a;
            congestionPG.Value = c;
            if (c >= 400) //Указываем размер склада
            {
        ...
        private void fromStorage3Btn_Click(object sender, EventArgs e)
        {
            goFromStorage();
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter();
        ...
        {
            SqlCommand goFromStrg1 = new SqlCommand("UPDATE StoragesCongestion SET Storage3 = Storage3 + @cubage where id=1", sqlConnection); //Указываем номер склада
            ...
            SqlCommand goFromStrg2 = new SqlCommand("DELETE FROM Storage3 WHERE id=@id", sqlConnection); //Указываем номер склада
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage3] WHERE (date1 LIKE '%" + dateSearchBox.Text + "%')", sqlConnection);
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage3] WHERE (keepingSumBox LIKE '%" + priceSearchBox.Text + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage3] WHERE (companyFrom LIKE N'%" + typeSearchBox.Text.ToString() + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
