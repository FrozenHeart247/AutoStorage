...
namespace AutoStorage
{
    public partial class Storage4 : MetroFramework.Forms.MetroForm
    {
        ...
        private void Storage4_Load(object sender, EventArgs e)
        {
            ...
            da.SelectCommand = new SqlCommand("SELECT * FROM Storage4", sqlConnection); //Указываем номер склада
            da.Fill(ds);
            ...
            store4CongPickerBox.Text = metroGrid2.CurrentRow.Cells[4].Value.ToString(); //указываем номер склада
            ...
            //////////////////////////////////////////
            int a = Convert.ToInt32(store4CongPickerBox.Text);
            int b = 1000; //Указываем размер склада
            int c;
            c = b - a;
            congestionPG.Value = c;
            if (c >= 1000) //Указываем размер склада
            {
        ...
        private void fromStorage4Btn_Click(object sender, EventArgs e)
        {
            goFromStorage();
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter();
        ...
        {
            SqlCommand goFromStrg1 = new SqlCommand("UPDATE StoragesCongestion SET Storage4 = Storage4 + @cubage where id=1", sqlConnection); //Указываем номер склада
            ...
            SqlCommand goFromStrg2 = new SqlCommand("DELETE FROM Storage4 WHERE id=@id", sqlConnection); //Указываем номер склада
        ...
        private void обновитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            da2.Fill(ds2);
            metroGrid2.DataSource = ds2.Tables[0];
            store4CongPickerBox.Text = metroGrid2.CurrentRow.Cells[4].Value.ToString();

        }

        private void dateSearchBox_KeyUp(object sender, KeyEventArgs e)
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage4] WHERE (date1 LIKE '%" + dateSearchBox.Text + "%')", sqlConnection);
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage4] WHERE (companyFrom LIKE N'%" + typeSearchBox.Text.ToString() + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
        ...
        {
            SqlCommand dateSearch = new SqlCommand("SELECT * FROM [Storage4] WHERE (keepingSumBox LIKE '%" + priceSearchBox.Text + "%')", sqlConnection);
            SqlDataReader dr = dateSearch.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
            metroGrid1.DataSource = dt;
